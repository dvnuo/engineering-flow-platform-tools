# Mermaid Flowchart Agent Guide

This public template accepts official Mermaid `flowchart` diagram syntax. Author Mermaid text in the `examples/basic.mmd` style and pass `.mmd` files to visual commands.

## When to use this template

Use this template when the requested diagram is a Mermaid Flowchart or an equivalent diagram that Mermaid documents under its Diagram Syntax.
...
---
flowchart LR
  User[User Browser] -->|request| Gateway[API Gateway]
  Gateway -->|calls| Service[Order Service]
  Service -->|writes| Database[(Order Database)]
  Service -->|emits| Queue[[Order Events]]
```
