# Mermaid Wardley Map Agent Guide

This public template accepts official Mermaid `wardley` diagram syntax. Author Mermaid text in the `examples/basic.mmd` style and pass `.mmd` files to visual commands.

## When to use this template

Use this template when the requested diagram is a Mermaid Wardley Map or an equivalent diagram that Mermaid documents under its Diagram Syntax.
...
```mermaid
---
title: Mermaid Wardley Example
---
wardley
  title Platform Map
  anchor User [0.95, 0.65]
  component Portal [0.75, 0.62]
  component Runtime [0.55, 0.50]
  component Cloud [0.25, 0.40]
  User->Portal
  Portal->Runtime
  Runtime->Cloud
```
