...
func visualExplicit(name string) (explicitMeta, bool) {
	...
	items := map[string]explicitMeta{
		"render": {Description: "Render a complete offline static visualization artifact from a local template and JSON input.",
			Flags: append([]string{"template", "input", "out", "title", "overwrite", "data-mode"}, common...), Required: []string{"template", "input", "out"}, Risk: "write", Example: "visual render --template uml.sequence_3d --template-dir ./templates/visual --input ./templates/visual/uml.sequence_3d/examples/game-session-flow.input.json --out ./out/sequence --json"},
