...
- If `inspect-image auth status --json` returns `token_state=refreshable`, `token_refreshable=true`, or `copilot_token_refreshable=true` for an explicitly configured Copilot provider, run `inspect-image auth test --json` or retry `inspect-image inspect --json`; do not ask the user to log in again.
