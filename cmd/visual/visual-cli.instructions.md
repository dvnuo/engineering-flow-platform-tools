...
- Each template also returns `visual_design` guidance. For large graph inputs, give every node a short `label` or `name`, use groups or node `parent_id`/`group_id`/`group`, start with an overview, and put noisy class/method/import edges behind `visibility: "detail"` or `visibility: "hidden"`.
