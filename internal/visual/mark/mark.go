...
func collectNodes(kind string, data map[string]any) []nodeItem {
	...
	case "studio_v1":
		...
		out := objectItemsFrom(heroData, "nodes", "$.hero.data")
		out = append(out, objectItemsFrom(heroData, "items", "$.hero.data")...)
		out = append(out, objectItemsFrom(heroData, "events", "$.hero.data")...)
		out = append(out, objectItemsFrom(heroData, "participants", "$.hero.data")...)
		return out
...
func collectEdges(kind string, data map[string]any) []edgeItem {
	...
	case "studio_v1":
		heroData := studioHeroData(data)
		out := edgeItemsFrom(heroData, "edges", "$.hero.data")
		out = append(out, edgeItemsFrom(heroData, "messages", "$.hero.data")...)
		out = append(out, edgeItemsFrom(heroData, "flows", "$.hero.data")...)
		out = append(out, edgeItemsFrom(heroData, "links", "$.hero.data")...)
		return out
	case "uml_sequence_v1":
...
func objectItems(data map[string]any, field string) []nodeItem {
	return objectItemsFrom(data, field, "$")
}

func objectItemsFrom(data map[string]any, field, prefix string) []nodeItem {
	raw, _ := data[field].([]any)
	out := make([]nodeItem, 0, len(raw))
	for i, item := range raw {
		if obj, ok := item.(map[string]any); ok {
			out = append(out, nodeItem{Path: prefix + "." + field + "[" + intString(i) + "]", Obj: obj})
		}
	}
	return out
}
...
func edgeItems(data map[string]any, field string) []edgeItem {
	return edgeItemsFrom(data, field, "$")
}

func edgeItemsFrom(data map[string]any, field, prefix string) []edgeItem {
	raw, _ := data[field].([]any)
	out := make([]edgeItem, 0, len(raw))
	for i, item := range raw {
		if obj, ok := item.(map[string]any); ok {
			out = append(out, edgeItem{Path: prefix + "." + field + "[" + intString(i) + "]", Obj: obj})
		}
	}
	return out
}
