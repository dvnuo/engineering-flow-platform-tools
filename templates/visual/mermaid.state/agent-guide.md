# Mermaid State Diagram Agent Guide

This public template accepts official Mermaid `state` diagram syntax. Author Mermaid text in the `examples/basic.mmd` style and pass `.mmd` files to visual commands.

## When to use this template

Use this template when the requested diagram is a Mermaid State Diagram or an equivalent diagram that Mermaid documents under its Diagram Syntax.
...
```mermaid
---
title: Mermaid State Diagram Example
---
stateDiagram-v2
  [*] --> Created
  Created --> Paid: payment accepted
  Paid --> Packed: allocate stock
