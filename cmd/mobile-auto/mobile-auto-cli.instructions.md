...
- Two distinct takeover shapes for sessions not started by mobile-auto (for example a Jenkins-driven Java/Cucumber/BrowserStack suite):
  - **Live takeover** (the job is still running and you need to interactively drive the app): find it with `--status running` (the default), then `mobile-auto run import --session-id ... --probe --json` (default `--probe=true` verifies the Appium hub still controls the session before writing local state).
  - **Post-mortem diagnosis** (the job already finished, pass or fail, and you only need its evidence): the session is no longer controllable, so pass `--status "" --probe=false` to skip both the "must still be running" filter and the live Appium check — `mobile-auto run import --from-url <browserstack-dashboard-url> --status "" --probe=false --json` (or `--session-id`/`--build`/`--name`). This still writes local run state so `mobile-auto run diagnose --run-id ... --collect-artifacts --json` can download `appiumlogs`, `devicelogs`, `crashlogs`, and `networklogs`, plus session video/build metadata — BrowserStack finalizes these only after the session ends, so a completed session's evidence is often more complete than a live one's. `mobile-auto session candidates --status "" --probe=false --json` lists candidates across every terminal status (`passed`, `failed`, `error`, `timeout`) the same way.
