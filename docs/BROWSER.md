...
The Portal page opens `efp-bridge://start?origin=<urlencoded Portal origin>&port=8765`; Windows then runs the hidden `browser.exe bridge-launch "<url>"`, which exits immediately when a bridge already answers `GET /ping` on `8765`-`8770` (first asking it to reopen its browser window through `session.ensure` when the ping reports `session.alive=false`, which is what a bridge looks like after the member closed the Chrome window) and otherwise starts `browser serve --origin <origin> --port <port>` as a detached background process (its output goes to `~/.efp/browser/logs/bridge-serve.log`).
