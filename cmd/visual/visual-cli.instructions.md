...
- For `architecture.isometric_overview`, use the same mark concepts on `entities[]` / `links[]`, not `nodes[]` / `edges[]`. Give entities `kind`, `provider`, `service`, `platform`, or `presentation.icon`; give links `directed=true`, concise `label`, `route[]` bend points, and `presentation.arrow=forward`.
