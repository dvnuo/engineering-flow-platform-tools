...
func browserLLMTips() []string {
	return []string{
		...
		"Use browser bookmark source list/add/update/remove to manage external source registrations in ~/.efp/config.yaml. External HTTP and HTTPS manifests are read-only; modify their bookmark entries in the owning system.",
		"browser bookmark list reads the local authoritative file and fetches external sources live on every call; it does not use or write a cache.",
