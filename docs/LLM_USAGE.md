...
- Use `browser bookmark add/update/remove` for the authoritative local source at `~/.efp/bookmarks.yaml`; removal requires user confirmation and `--yes`. Use `browser bookmark source list/add/update/remove` for external source registrations in the shared EFP config. External HTTP/HTTPS manifests are read-only and must be changed in their owning system. `bookmark list` merges local entries first with live external results and does not use a cache.
