...
func browserExplicit(name string) (explicitMeta, bool) {
	...
	items := map[string]explicitMeta{
		...
		"session.start": {Description: "Start a persistent Edge/Chrome/Chromium automation session that remains open for manual login and page operations in later turns, with DevTools bound to 127.0.0.1.",
			...
			Lifecycle: "persistent", WhenToUse: "Use when explicitly creating and configuring a persistent browser session for manual login or page operations in later turns.", WhenNotToUse: "Do not use as the general open-or-reuse entry point when the session may already exist; use browser open for the unified persistent open contract."},
