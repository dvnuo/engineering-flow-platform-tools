# Mermaid ZenUML Sequence Agent Guide

This public template accepts official Mermaid `zenuml` diagram syntax. Author Mermaid text in the `examples/basic.mmd` style and pass `.mmd` files to visual commands.

## When to use this template

Use this template when the requested diagram is a Mermaid ZenUML Sequence or an equivalent diagram that Mermaid documents under its Diagram Syntax.
...
```mermaid
---
title: Mermaid ZenUML Example
---
zenuml
  title Checkout Flow
  User->Portal: submit order
  Portal->API: create order
  API->Database: write order
  API-->Portal: created
```
