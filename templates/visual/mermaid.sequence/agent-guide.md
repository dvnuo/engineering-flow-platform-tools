# Mermaid Sequence Diagram Agent Guide

This public template accepts official Mermaid `sequence` diagram syntax. Author Mermaid text in the `examples/basic.mmd` style and pass `.mmd` files to visual commands.

## When to use this template

Use this template when the requested diagram is a Mermaid Sequence Diagram or an equivalent diagram that Mermaid documents under its Diagram Syntax.
...
sequenceDiagram
  ...
  API->>Database: persist order
  Database-->>API: order id
  API-->>Portal: order created
