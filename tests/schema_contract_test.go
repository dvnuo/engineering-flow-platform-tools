...
func TestSchemaConcreteFlags(t *testing.T) {
	...
	requireFlags(t, schemaData(t, "jira", "zephyr.execution.add-tests-to-cycle"), "cycle-id", "project-id", "version-id", "issues", "folder-id", "folder-name", "create-folder", "dry-run")
	requireFlags(t, schemaData(t, "jira", "zephyr.execution.bulk-update-status"), "execution-ids", "cycle-id", "issues", "folder-id", "folder-name", "status", "dry-run")
	requireFlags(t, schemaData(t, "jira", "zephyr.archive.executions"), "execution-ids", "issues", "cycle-id", "folder-id", "folder-name", "yes", "dry-run")
	...
	requireFlags(t, schemaData(t, "browser", "page.find"), "selector", "role", "name", "text", "label", "placeholder", "near-text", "nth", "limit", "include-hidden", "session", "target-id", "timeout")
	requireFlags(t, schemaData(t, "browser", "page.ax"), "limit", "include-hidden", "pierce", "session", "target-id", "timeout")
	requireFlags(t, schemaData(t, "browser", "page.click"), "selector", "ref", "yes", "session", "target-id", "timeout")
	...
	requireFlags(t, schemaData(t, "browser", "page.table-export"), "selector", "out", "format", "limit-rows", "limit-cells", "session", "target-id", "timeout")
	requireRequired(t, schemaData(t, "browser", "page.table-export"), "out")
	requireFlags(t, schemaData(t, "browser", "page.list"), "selector", "limit-items", "session", "target-id", "timeout")
	requireFlags(t, schemaData(t, "browser", "page.list-export"), "selector", "out", "format", "limit-items", "session", "target-id", "timeout")
	requireRequired(t, schemaData(t, "browser", "page.list-export"), "out")
	requireFlags(t, schemaData(t, "browser", "page.scroll-collect"), "selector", "item-selector", "out", "format", "limit", "max-scrolls", "scroll-step", "interval-ms", "session", "target-id", "timeout")
	requireFlags(t, schemaData(t, "browser", "page.diff"), "before", "after", "out", "limit")
	requireRequired(t, schemaData(t, "browser", "page.diff"), "before", "after")
	...
	requireFlags(t, schemaData(t, "browser", "workflow.run"), "file", "dry-run", "session", "target-id", "timeout", "continue-on-error", "var", "report-out", "evidence-dir", "allow-human", "yes")
	...
	requireFlags(t, schemaData(t, "browser", "network.start"), "session", "target-id", "timeout", "limit", "filter", "body", "max-body-bytes")
	requireFlags(t, schemaData(t, "browser", "network.stop"), "session", "target-id", "timeout")
	requireFlags(t, schemaData(t, "browser", "network.list"), "session", "target-id", "timeout", "filter", "limit", "method", "status", "body", "max-body-bytes")
	requireFlags(t, schemaData(t, "browser", "network.wait"), "session", "target-id", "timeout", "url-contains", "method", "status", "limit", "body", "max-body-bytes")
