...
- Use `browser bookmark add/update/remove` for the authoritative local source at `~/.efp/bookmarks.yaml`; removal requires user confirmation and `--yes`. Use `browser bookmark source list/add/update/remove` for read-only HTTP/HTTPS or local file source registrations in the shared EFP config. Change bookmark entries in the source manifest itself. `bookmark list` merges local entries first with live configured results and does not use a cache.
