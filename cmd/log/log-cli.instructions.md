...
- Then use `log profile`, `log template list`, `log group`, `log timeline`, `log search`, `log window`, `log extract`, and `log summarize` against that run directory.
...
log template list ./.log-runs/run_001 --only non-info --sort count --limit 20 --json
log group ./.log-runs/run_001 --by error_signature --limit 20 --json
