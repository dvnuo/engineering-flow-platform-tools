...
func browserExplicit(name string) (explicitMeta, bool) {
	...
	items := map[string]explicitMeta{
		...
		"bookmark.list": {Description: "Read authoritative local bookmarks, load configured HTTP/HTTPS or local file sources live, and merge website names, aliases, required descriptions, URLs, and source names for agent routing.",
		...
		"bookmark.add": {Description: "Add a website bookmark to the authoritative local source at ~/.efp/bookmarks.yaml; configured HTTP/HTTPS and local file sources remain read-only.",
			...
			WhenToUse: "Use when the user wants to save a personal local website bookmark.", WhenNotToUse: "Do not use to modify a configured source manifest; change the manifest itself."},
		"bookmark.update": {Description: "Update selected fields of a website bookmark in the authoritative local source; configured HTTP/HTTPS and local file sources remain read-only.",
			Flags: append([]string{"source", "name", "alias", "clear-aliases", "description", "url"}, common...), Risk: "write", Example: `browser bookmark update Google --description "Search public websites." --json`,
			WhenToUse: "Use when the user wants to change a personal local bookmark.", WhenNotToUse: "Do not use to modify a configured source manifest; change the manifest itself."},
		"bookmark.remove": {Description: "Remove a website bookmark from the authoritative local source after explicit confirmation.",
			...
			WhenToUse: "Use after the user explicitly confirms removal of a local bookmark.", WhenNotToUse: "Do not use to remove an entry from a configured source manifest."},
		"bookmark.source.list": {Description: "List read-only HTTP/HTTPS or local file bookmark source registrations from browser.bookmarks.sources without loading their manifests.",
		...
		"bookmark.source.add": {Description: "Register a read-only HTTP/HTTPS or local file bookmark manifest in the shared EFP config.",
			Flags: append([]string{"name", "url"}, common...), Required: []string{"name", "url"}, Risk: "write", Example: "browser bookmark source add --name team --url ~/.efp/team-bookmarks.yaml --json",
			WhenToUse: "Use when the user wants browser bookmark list to include another remote or local manifest.", WhenNotToUse: "Do not use this to add an individual bookmark; use browser bookmark add for the local source."},
		"bookmark.source.update": {Description: "Update the name or location of a read-only HTTP/HTTPS or local file bookmark source registration in the shared EFP config.",
			Flags: append([]string{"name", "url"}, common...), Risk: "write", Example: "browser bookmark source update company --url https://portal.example.test/new-bookmarks.yaml --json",
			WhenToUse: "Use when the user wants to rename a registered source or change its manifest URL or file path.", WhenNotToUse: "Do not imply that this changes entries inside the source manifest."},
