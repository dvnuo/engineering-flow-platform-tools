# Mermaid C4 Diagram Agent Guide

This public template accepts official Mermaid `c4` diagram syntax. Author Mermaid text in the `examples/basic.mmd` style and pass `.mmd` files to visual commands.

## When to use this template

Use this template when the requested diagram is a Mermaid C4 Diagram or an equivalent diagram that Mermaid documents under its Diagram Syntax.
...
C4Context
  ...
  SystemDb(db, "Order DB")
  Rel(user, portal, "uses")
  Rel(portal, api, "calls")
  Rel(api, db, "writes")
```
