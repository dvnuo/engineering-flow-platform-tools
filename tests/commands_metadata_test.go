...
func TestCommandsMetadataComplete(t *testing.T) {
	...
	for name, root := range map[string]*cobra.Command{"jira": jcmd.NewRoot(), "confluence": ccmd.NewRoot(), "jenkins": kcmd.NewRoot(), "aws-auth": acmd.NewRoot(), "browser": bcmd.NewRoot(), "inspect-image": icmd.NewRoot()} {
