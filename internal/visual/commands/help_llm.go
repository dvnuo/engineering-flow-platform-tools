...
func visualLLMTips() []string {
	return []string{
		...
		"Always inspect templates with visual template list and visual template get before rendering.",
		"Use --template-dir when templates live in the current workspace or release artifact.",
		"Generate input JSON that matches the selected template input_schema_kind.",
