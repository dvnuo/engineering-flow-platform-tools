...
func browserLLMTips() []string {
	return []string{
		...
		"Use browser workflow run --file flow.yaml --dry-run --json before executing YAML workflows; workflows support variables, conditions, for_each expansion, smart_wait, human.wait/confirm, report-out audit logs, and whitelisted browser actions/assertions only.",
