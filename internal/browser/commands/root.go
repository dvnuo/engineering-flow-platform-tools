...
func browserLLMTips() []string {
	return []string{
		...
		"Choose a bookmark by matching the user's intent against its name, aliases, and required description, then pass that bookmark's returned URL unchanged to browser open --url <url> --json.",
		...
		"Use the default session for normal workflows. Omit --session on browser open, tab, page, assert, form, frame, network, download, and workflow commands; the CLI defaults it to default.",
		"Do not invent a session name from the task, website, project, URL, or requested action. Use a non-default session only when the user explicitly names it, a prior successful browser command established it, or the user explicitly requests an isolated concurrent session.",
		"When login state may be required, use the default session from the first command; do not try a task-specific session and then fall back to default after discovering that login is needed.",
