...
Configuration uses the shared EFP config from environment variables injected by managed runtimes (for example EFP_NEXUS_DEFAULT_INSTANCE, EFP_NEXUS_INSTANCES_0_BASE_URL) or the config file, normally ~/.efp/config.yaml (local), under the nexus node.`),
		Examples: []string{
			`nexus commands --json`,
			`nexus help llm --json`,
			`nexus version --json`,
