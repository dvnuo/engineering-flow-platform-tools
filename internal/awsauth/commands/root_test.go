...
func TestLoadAWSConfigForReadPrefersEnv(t *testing.T) {
	...
	if cfg.AWS.Domain != "HBEU" || cfg.AWS.Username != "env-user" || cfg.AWS.Password != "env-pass" {
		t.Fatalf("aws config not taken from env blob: %#v", cfg.AWS)
	}
}

func TestAuthLoginRefusedWhenEnvManaged(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	t.Setenv("USERPROFILE", home)
	t.Setenv(config.EnvConfigPath, "")
	...
	if errObj == nil || errObj["code"] != "config_env_managed" {
		t.Fatalf("expected config_env_managed, got %#v", obj)
	}
	if _, err := os.Stat(filepath.Join(home, ".efp", "config.yaml")); !os.IsNotExist(err) {
		t.Fatalf("no file must be written when refused: %v", err)
	}
}
