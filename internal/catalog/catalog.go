...
func browserExplicit(name string) (explicitMeta, bool) {
	...
	items := map[string]explicitMeta{
		...
		"bookmark.list": {Description: "Read authoritative local bookmarks, fetch configured external sources live, and merge website names, aliases, required descriptions, URLs, and source names for agent routing.",
		...
		"bookmark.add": {Description: "Add a website bookmark to the authoritative local source at ~/.efp/bookmarks.yaml; external HTTP and HTTPS sources remain read-only.",
			Flags: append([]string{"source", "name", "alias", "description", "url"}, common...), Required: []string{"name", "description", "url"}, Risk: "write", Example: `browser bookmark add --name Google --alias 谷歌 --description "Search the public web." --url https://www.google.com/ --json`,
			WhenToUse: "Use when the user wants to save a personal local website bookmark.", WhenNotToUse: "Do not use to modify an external source manifest; change it in its owning system."},
		"bookmark.update": {Description: "Update selected fields of a website bookmark in the authoritative local source; external HTTP and HTTPS sources remain read-only.",
			Flags: append([]string{"source", "name", "alias", "clear-aliases", "description", "url"}, common...), Required: []string{"name", "one update flag"}, Risk: "write", Example: `browser bookmark update Google --description "Search public websites." --json`,
			WhenToUse: "Use when the user wants to change a personal local bookmark.", WhenNotToUse: "Do not use to modify an external source manifest; change it in its owning system."},
		"bookmark.remove": {Description: "Remove a website bookmark from the authoritative local source after explicit confirmation.",
			Flags: append([]string{"source", "yes"}, common...), Required: []string{"name", "yes"}, Risk: "delete", Example: "browser bookmark remove Google --yes --json",
			WhenToUse: "Use after the user explicitly confirms removal of a local bookmark.", WhenNotToUse: "Do not use to remove an entry from an external source manifest."},
		"bookmark.source.list": {Description: "List external read-only bookmark source registrations from browser.bookmarks.sources without fetching their manifests.",
			Flags: common, Risk: "read", Example: "browser bookmark source list --json",
			WhenToUse: "Use when the user wants to inspect which external bookmark manifests are registered.", WhenNotToUse: "Do not use this instead of bookmark list when resolving a website; source list does not fetch bookmark entries."},
		"bookmark.source.add": {Description: "Register an external read-only HTTP or HTTPS bookmark manifest in the shared EFP config.",
			Flags: append([]string{"name", "url"}, common...), Required: []string{"name", "url"}, Risk: "write", Example: "browser bookmark source add --name company --url https://portal.example.test/bookmarks.yaml --json",
			WhenToUse: "Use when the user wants browser bookmark list to include another external manifest.", WhenNotToUse: "Do not use this to add an individual bookmark; use browser bookmark add for the local source."},
		"bookmark.source.update": {Description: "Update the name or URL of an external read-only bookmark source registration in the shared EFP config.",
			Flags: append([]string{"name", "url"}, common...), Required: []string{"name", "name|url"}, Risk: "write", Example: "browser bookmark source update company --url https://portal.example.test/new-bookmarks.yaml --json",
			WhenToUse: "Use when the user wants to rename a registered external source or change its manifest URL.", WhenNotToUse: "Do not imply that this changes entries inside the remote manifest."},
		"bookmark.source.remove": {Description: "Remove an external bookmark source registration after explicit confirmation without modifying the remote manifest.",
			...
			WhenToUse: "Use after the user explicitly confirms that an external source registration should be removed.", WhenNotToUse: "Do not imply that this deletes the remote manifest or its bookmarks."},
