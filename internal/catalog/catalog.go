...
var mobileAutoCommands = []string{
	...
	"mobile-auto session list", "mobile-auto session search", "mobile-auto session probe", "mobile-auto session candidates", "mobile-auto session get", "mobile-auto session mark", "mobile-auto session start", "mobile-auto session status", "mobile-auto session stop",
	"mobile-auto run start", "mobile-auto run status", "mobile-auto run recover", "mobile-auto run import", "mobile-auto run guard", "mobile-auto run diagnose", "mobile-auto run claim", "mobile-auto run release", "mobile-auto run monitor", "mobile-auto run handoff", "mobile-auto run resume", "mobile-auto run finish",
...
func mobileAutoExplicit(name string) (explicitMeta, bool) {
	...
	items := map[string]explicitMeta{
		...
		"run.recover":    {Description: "Recover local mobile-auto run state from an existing BrowserStack session or build after interrupted session creation.", Flags: append([]string{"run-id", "session-id", "build-id", "build", "name", "network", "local-identifier", "platform", "app"}, common...), Required: []string{"session-id|build-id|build"}, Risk: "write", Example: "mobile-auto run recover --session-id session-123 --json"},
		"run.import":     {Description: "Import an existing BrowserStack App Automate session into local mobile-auto run state after optional Appium control probing.", Flags: append([]string{"run-id", "session-id", "build-id", "build", "name", "status", "project-id", "network", "local-identifier", "platform", "device", "app", "from-url", "probe", "dry-run"}, common...), Required: []string{"session-id|build-id|build|from-url"}, Risk: "write", Example: "mobile-auto run import --session-id session-123 --build-id build-123 --json"},
		"run.guard":      {Description: "Start a bounded keepalive guard for an active run so imported or handed-off sessions are less likely to idle out.", Flags: append([]string{"run-id", "hold-for"}, common...), Required: []string{"run-id"}, Risk: "write", Example: "mobile-auto run guard --run-id run-123 --hold-for 10m --json"},
		"run.diagnose":   {Description: "Write a diagnostic bundle for a run with probe status, timeline, optional observation, capacity data, and BrowserStack artifacts.", Flags: append([]string{"run-id", "out", "collect-artifacts", "observe"}, common...), Required: []string{"run-id"}, Risk: "read", Example: "mobile-auto run diagnose --run-id run-123 --out evidence --json"},
		"run.claim":      {Description: "Record a local control lease for a run while acknowledging BrowserStack does not enforce remote exclusivity.", Flags: append([]string{"run-id", "owner", "ttl"}, common...), Required: []string{"run-id"}, Risk: "write", Example: "mobile-auto run claim --run-id run-123 --owner agent --ttl 15m --json"},
		"run.release":    {Description: "Clear the local control lease recorded for a run.", Flags: append([]string{"run-id"}, common...), Required: []string{"run-id"}, Risk: "write", Example: "mobile-auto run release --run-id run-123 --json"},
		"run.monitor":    {Description: "Capture one or more BrowserStack capacity and running-session snapshots for operational monitoring.", Flags: append([]string{"duration", "poll-interval", "status", "project-id"}, common...), Risk: "read", Example: "mobile-auto run monitor --duration 0s --status running --json"},
		"session.search": {Description: "Search BrowserStack App Automate sessions across recent builds using status, build, project, session, platform, and device filters.", Flags: append([]string{"build-id", "build", "name", "status", "project-id", "platform", "device", "limit", "offset"}, common...), Risk: "read", Example: "mobile-auto session search --status running --json"},
		"session.probe":  {Description: "Probe one BrowserStack session for control-plane visibility, remote status, and Appium controllability.", Flags: append([]string{"session-id", "build-id", "build", "name", "status", "project-id", "platform", "device", "from-url", "probe", "deep"}, common...), Required: []string{"session-id|build-id|build|from-url"}, Risk: "read", Example: "mobile-auto session probe --session-id session-123 --json"},
		"session.candidates": {Description: "List running BrowserStack sessions with importability, Appium probe result, and a recommended import command.",
			Flags: append([]string{"build-id", "build", "name", "status", "project-id", "platform", "device", "limit", "offset", "probe", "deep"}, common...), Risk: "read", Example: "mobile-auto session candidates --status running --json"},
...
func browserExplicit(name string) (explicitMeta, bool) {
	...
	items := map[string]explicitMeta{
		"open": {Description: "Open an HTTP or HTTPS URL in a persistent Edge/Chrome/Chromium session that remains available for manual login and page operations in later turns.",
			Flags: []string{"url", "session", "browser", "browser-exe", "headless", "profile", "download-dir", "clean-profile", "port", "config", "json", "format", "verbose"}, Required: []string{"url"}, Risk: "write", Example: "browser open --url https://intranet.example.test --json",
		...
		"bookmark.list": {Description: "Load configured HTTP/HTTPS or local file bookmark sources live and merge website names, aliases, required descriptions, URLs, and source names for agent routing; repeat --source to filter by source name.",
			Flags: append([]string{"source"}, common...), Risk: "read", Example: "browser bookmark list --source personal --json",
		...
		"bookmark.add": {Description: "Add a website bookmark to the configured local file source selected by --source; create its manifest and parent directory on first write.",
			Flags: append([]string{"source", "name", "alias", "description", "url"}, common...), Required: []string{"source", "name", "description", "url"}, Risk: "write", Example: `browser bookmark add --source personal --name "Example Search" --alias "web search" --description "Search example content." --url https://search.example.test/ --json`,
			WhenToUse: "Use when the user wants to save a bookmark in a configured local file source.", WhenNotToUse: "Do not use with an HTTP/HTTPS source; remote manifests are read-only."},
		"bookmark.update": {Description: "Update selected fields of a website bookmark in the configured local file source selected by --source.",
			Flags: append([]string{"source", "name", "alias", "clear-aliases", "description", "url"}, common...), Required: []string{"source"}, Risk: "write", Example: `browser bookmark update "Example Search" --source personal --description "Search example websites." --json`,
			WhenToUse: "Use when the user wants to change a bookmark in a configured local file source.", WhenNotToUse: "Do not use with an HTTP/HTTPS source; remote manifests are read-only."},
		"bookmark.remove": {Description: "Remove a website bookmark from the configured local file source selected by --source after explicit confirmation.",
			Flags: append([]string{"source", "yes"}, common...), Required: []string{"name", "source", "yes"}, Risk: "delete", Example: `browser bookmark remove "Example Search" --source personal --yes --json`,
			WhenToUse: "Use after the user explicitly confirms removal of a bookmark from a configured local file source.", WhenNotToUse: "Do not use with an HTTP/HTTPS source; remote manifests are read-only."},
		"bookmark.source.list": {Description: "List HTTP/HTTPS or local file bookmark source registrations, including optional descriptions, from browser.bookmarks.sources without loading their manifests.",
			Flags: common, Risk: "read", Example: "browser bookmark source list --json",
			WhenToUse: "Use when the user wants to inspect which remote or local bookmark manifests are registered.", WhenNotToUse: "Do not use this instead of bookmark list when resolving a website; source list does not load bookmark entries."},
		"bookmark.source.add": {Description: "Register an HTTP/HTTPS or local file bookmark manifest with an optional source description in the shared EFP config.",
			Flags: append([]string{"name", "description", "url"}, common...), Required: []string{"name", "url"}, Risk: "write", Example: `browser bookmark source add --name personal --description "Personal websites." --url ~/.efp/browser/bookmarks/personal.yaml --json`,
			WhenToUse: "Use when the user wants browser bookmark list to include another remote or local manifest.", WhenNotToUse: "Do not use this to add an individual bookmark; use browser bookmark add with --source."},
		"bookmark.source.update": {Description: "Update the name, description, or location of an HTTP/HTTPS or local file bookmark source registration in the shared EFP config.",
			Flags: append([]string{"name", "description", "url"}, common...), Risk: "write", Example: `browser bookmark source update personal --description "Personal search and productivity websites." --json`,
			WhenToUse: "Use when the user wants to rename a registered source or change its description, manifest URL, or file path.", WhenNotToUse: "Do not imply that this changes entries inside the source manifest."},
		"bookmark.source.remove": {Description: "Remove a bookmark source registration after explicit confirmation without modifying its remote or local manifest.",
			Flags: append([]string{"yes"}, common...), Required: []string{"name", "yes"}, Risk: "delete", Example: "browser bookmark source remove company --yes --json",
			WhenToUse: "Use after the user explicitly confirms that a source registration should be removed.", WhenNotToUse: "Do not imply that this deletes the source manifest or its bookmarks."},
		...
		"tab.current": {Description: "Show the active page tab for a browser automation session.",
			Flags: sessionFlag, Risk: "read", Example: "browser tab current --json"},
		...
		"page.snapshot": {Description: "Return redacted URL, title, body text preview, and optional HTML preview from the selected page target.",
			Flags: append([]string{"include-html", "max-text-bytes", "max-html-bytes"}, pageFlags...), Risk: "read", Example: "browser page snapshot --json"},
		...
		"serve": {Description: "Serve the EFP Portal local browser bridge on 127.0.0.1 so a Portal page can run tab and page commands against the managed browser session; not for interactive agent use.",
			Flags: []string{"origin", "port", "session", "browser", "browser-exe", "headless", "url", "register-protocol", "unregister-protocol", "config", "json", "format", "verbose"}, Required: []string{"origin"}, Risk: "write", Example: "browser serve --origin https://portal.example.test --port 8765 --json",
			Lifecycle: "persistent", WhenToUse: "Use only when the EFP Portal local browser connector needs a bridge process on the user's machine; it is normally started by install-bridge.cmd or the efp-bridge:// protocol link.", WhenNotToUse: "Do not use from an interactive agent session to read or act on pages; use browser open and the page commands directly."},
...
func inspectImageExplicit(name string) (explicitMeta, bool) {
	items := map[string]explicitMeta{
		"inspect": {Description: "Inspect exactly one local image using the configured vision provider.",
		...
		"auth.login": {Description: "Authenticate inspect-image with the configured provider.",
		...
		"doctor": {Description: "Check inspect-image config, auth, proxy mode, provider endpoint, and model defaults.",
			Flags: []string{"config", "json", "format", "verbose"}, Risk: "read", Example: "inspect-image doctor --json"},
		"models": {Description: "Show inspect-image model default and allowed reasoning efforts.",
...
func jenkinsExplicit(name string) (explicitMeta, bool) {
	...
	items := map[string]explicitMeta{
		...
		"build.test-report": {Description: "Fetch a Jenkins build's JUnit-shaped test report (JUnit, Cucumber-JVM JUnit formatter, most CI test reporters) normalized to a failures list; returns has_report=false instead of erroring when no report is published.",
			Flags: append([]string{"max-failures"}, common...), Required: []string{"job", "build"}, Risk: "read", Example: "jenkins build test-report folder/app-main 42 --json"},
		"build.wait": {Description: "Poll a Jenkins build until it stops building, or time out; returns the terminal result state.",
			Flags: append([]string{"interval-ms", "timeout-sec"}, common...), Required: []string{"job", "build"}, Risk: "read", Example: "jenkins build wait folder/app-main 42 --timeout-sec 900 --json"},
